...
interface ImportError {
    row: number;
    data: { nama: string; inisial: string };
    messages: string[];
}
...
export default function BulkCreate({ importResults }: PageProps<Props>) {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        if (!data.file) return;
        post(route('admin.dosen.bulk.store'), {
            onSuccess: () => {
    ...
    const downloadTemplate = () => {
        window.location.href = route('admin.dosen.bulk.template');
    };

    return (
        <AppLayout>
            <Head title="Tambah Dosen (Bulk)" />

            <div className="space-y-6 w-full">
                <div className="flex items-center gap-3">
                    <Button variant="outline" size="icon" onClick={() => router.visit(route('admin.dosen.index'))}>
                        <ArrowLeft className="h-4 w-4" />
                    </Button>
                    <h1 className="text-2xl font-bold">Tambah Dosen (Bulk)</h1>
                </div>
                        ...
                        <div className="rounded-md border">
                            <Table>
                                ...
                                <TableBody>
                                    ...
                                    <TableRow>
                                        <TableCell className="text-muted-foreground text-xs">Nama lengkap dosen (wajib)</TableCell>
                                        <TableCell className="text-muted-foreground text-xs">Inisial, maks 10 karakter (wajib, unik)</TableCell>
                                    </TableRow>
                                </TableBody>
                            </Table>
                        </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Pilih file Excel (.xlsx, .xls) atau CSV yang berisi data dosen.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={submit} className="space-y-4">
                            ...
                            <div className="flex gap-2">
                                ...
                                <Button variant="outline" type="button" onClick={() => router.visit(route('admin.dosen.index'))}>
                                    Batal
                                </Button>
                            </div>
                        </form>
                ...
                {importResults && (
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="flex gap-4">
                                {importResults.validCount > 0 && (
                                    <Alert>
                                        ...
                                        <AlertDescription>
                                            {importResults.validCount} dosen berhasil ditambahkan.
                                        </AlertDescription>
                                    </Alert>
                                )}
                                        ...
                                        <TableBody>
                                            {importResults.errors.map((err, i) => (
                                                <TableRow key={i}>
                                                    ...
                                                    <TableCell>{err.data.inisial || '-'}</TableCell>
                            ...
                            {importResults.validCount > 0 && (
                                <div className="flex gap-2">
                                    <Button onClick={() => router.visit(route('admin.dosen.index'))}>
                                        Lihat Data Dosen
                                    </Button>
                                </div>
                            )}
