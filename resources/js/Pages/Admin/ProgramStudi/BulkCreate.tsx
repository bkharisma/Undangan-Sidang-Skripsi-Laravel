...
export default function BulkCreate({ importResults }: PageProps<Props>) {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        if (!data.file) return;
        post(route('admin.program-studi.bulk.store'), {
            onSuccess: () => {
    ...
    const downloadTemplate = () => {
        window.location.href = route('admin.program-studi.bulk.template');
    };

    return (
        <AppLayout>
            <Head title="Tambah Program Studi (Bulk)" />

            <div className="space-y-6 max-w-2xl">
                <div className="flex items-center gap-3">
                    <Button variant="outline" size="icon" onClick={() => router.visit(route('admin.program-studi.index'))}>
                        <ArrowLeft className="h-4 w-4" />
                    </Button>
                    <h1 className="text-2xl font-bold">Tambah Program Studi (Bulk)</h1>
                </div>
                        ...
                        <div className="rounded-md border">
                            <Table>
                                ...
                                <TableBody>
                                    ...
                                    <TableRow>
                                        <TableCell className="text-muted-foreground text-xs">Kode, maks 20 karakter (wajib, unik)</TableCell>
                                        <TableCell className="text-muted-foreground text-xs">Nama program studi (wajib)</TableCell>
                                    </TableRow>
                                </TableBody>
                            </Table>
                        </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Pilih file Excel (.xlsx, .xls) atau CSV yang berisi data program studi.</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                {importResults && (
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="flex gap-4">
                                {importResults.validCount > 0 && (
                                    <Alert>
                                        ...
                                        <AlertDescription>
                                            {importResults.validCount} program studi berhasil ditambahkan.
                                        </AlertDescription>
                                    </Alert>
                                )}
                                        ...
                                        <TableBody>
                                            {importResults.errors.map((err, i) => (
                                                <TableRow key={i}>
                                                    ...
                                                    <TableCell>{err.data.kode || '-'}</TableCell>
                            ...
                            {importResults.validCount > 0 && (
                                <div className="flex gap-2">
                                    <Button onClick={() => router.visit(route('admin.program-studi.index'))}>
