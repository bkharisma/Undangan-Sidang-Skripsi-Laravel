...
export default function BulkCreate({ importResults }: PageProps<Props>) {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        if (!data.file) return;
        post(route('admin.pic.bulk.store'), {
            onSuccess: () => {
    ...
    const downloadTemplate = () => {
        window.location.href = route('admin.pic.bulk.template');
    };

    return (
        <AppLayout>
            <Head title="Tambah PIC (Bulk)" />

            <div className="space-y-6 w-full">
                <div className="flex items-center gap-3">
                    <Button variant="outline" size="icon" onClick={() => router.visit(route('admin.pic.index'))}>
                        <ArrowLeft className="h-4 w-4" />
                    </Button>
                    <h1 className="text-2xl font-bold">Tambah PIC (Bulk)</h1>
                </div>
                        ...
                        <div className="rounded-md border">
                            <Table>
                                ...
                                <TableBody>
                                    ...
                                    <TableRow>
                                        <TableCell className="text-muted-foreground text-xs">Nama PIC (wajib)</TableCell>
                                    </TableRow>
                                </TableBody>
                            </Table>
                        </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Pilih file Excel (.xlsx, .xls) atau CSV yang berisi data PIC.</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                {importResults && (
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="flex gap-4">
                                {importResults.validCount > 0 && (
                                    <Alert>
                                        ...
                                        <AlertDescription>
                                            {importResults.validCount} PIC berhasil ditambahkan.
                                        </AlertDescription>
                                    </Alert>
                                )}
                            ...
                            {importResults.validCount > 0 && (
                                <div className="flex gap-2">
                                    <Button onClick={() => router.visit(route('admin.pic.index'))}>
                                        Lihat Data PIC
                                    </Button>
                                </div>
                            )}
