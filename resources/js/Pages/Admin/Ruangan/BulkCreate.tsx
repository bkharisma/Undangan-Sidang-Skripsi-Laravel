...
export default function BulkCreate({ importResults }: PageProps<Props>) {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        if (!data.file) return;
        post(route('admin.ruangan.bulk.store'), {
            onSuccess: () => {
    ...
    const downloadTemplate = () => {
        window.location.href = route('admin.ruangan.bulk.template');
    };

    return (
        <AppLayout>
            <Head title="Tambah Ruangan (Bulk)" />

            <div className="space-y-6 w-full">
                <div className="flex items-center gap-3">
                    <Button variant="outline" size="icon" onClick={() => router.visit(route('admin.ruangan.index'))}>
                        <ArrowLeft className="h-4 w-4" />
                    </Button>
                    <h1 className="text-2xl font-bold">Tambah Ruangan (Bulk)</h1>
                </div>
                        ...
                        <div className="rounded-md border">
                            <Table>
                                ...
                                <TableBody>
                                    ...
                                    <TableRow>
                                        <TableCell className="text-muted-foreground text-xs">Nama ruangan (wajib)</TableCell>
                                    </TableRow>
                                </TableBody>
                            </Table>
                        </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Pilih file Excel (.xlsx, .xls) atau CSV yang berisi data ruangan.</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                {importResults && (
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="flex gap-4">
                                {importResults.validCount > 0 && (
                                    <Alert>
                                        ...
                                        <AlertDescription>
                                            {importResults.validCount} ruangan berhasil ditambahkan.
                                        </AlertDescription>
                                    </Alert>
                                )}
                                        ...
                                        <TableBody>
                                            {importResults.validIds.map((id, i) => (
                                                <TableRow key={i}>
                                                    ...
                                                    <TableCell className="text-muted-foreground text-xs">Gunakan ID ini sebagai referensi di Excel Sidang (kolom H)</TableCell>
                                                </TableRow>
                                            ))}
                                        </TableBody>
                                    </Table>
                            ...
                            {importResults.validCount > 0 && (
                                <div className="flex gap-2">
                                    <Button onClick={() => router.visit(route('admin.ruangan.index'))}>
                                        Lihat Data Ruangan
                                    </Button>
                                </div>
                            )}
